...
import { useUploadAccounting, useAccountingEntries, useDeleteAccounting } from '../hooks/useApi'
...
export default function AccountingPage() {
  ...
  const uploadMut = useUploadAccounting()
  const { data: result, isLoading } = useAccountingEntries(page, filters, sort)
  const deleteMut = useDeleteAccounting()
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      const saved = JSON.parse(localStorage.getItem('accounting_column_mapping') || '{}')
      const restored = {}
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // 保存列匹配信息
      localStorage.setItem('accounting_column_mapping', JSON.stringify(mapping))
      toast.success('上传成功')
      setFile(null)
      setHeaders([])
  ...
  return (
    <div>
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        <h2 className="text-lg font-semibold mb-4">上传记账单</h2>
        <p className="text-sm text-gray-500 mb-4">支持读取3个sheet：支出、转账、收入</p>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold">记账单列表 ({total})</h2>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="divide-y divide-gray-200">
                {entries.map((entry) => (
                  <tr key={entry.id} className={entry.bankTransaction ? 'bg-green-50' : ''}>
                    <td className="px-4 py-3 text-sm font-mono text-gray-500">{entry.rowNo || '-'}</td>
